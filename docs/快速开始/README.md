...
| 目标 | 先做什么 | 再看哪里 |
|------|----------|----------|
| 只体验系统 | 访问在线演示或按最小路径启动本地环境 | [用户教程](../用户教程/README.md) |
| 做接口联调 | 本地启动后端并登录获取 Token | [接口参考](../接口参考/README.md)、[在线测试](../接口参考/在线测试.md) |
| 新增业务功能 | 确认后端、前端和数据库都可启动 | [开发指南](../开发指南/README.md)、[新模块开发](../开发指南/新模块开发.md) |
| 开发插件 | 使用源码环境，不在发布二进制里动态安装插件 | [插件开发](../开发指南/插件开发.md)、[插件实战教程](../开发指南/插件实战教程.md) |
| 准备上线 | 不使用无参数 watch 入口托管生产服务 | [生产部署](../部署运维/生产部署.md)、[发布升级](../部署运维/发布升级.md) |
...
公开文档会展示会员授权插件的接口和使用说明，便于评估与联调；这不表示开源版已经包含对应源码或 ZIP。插件是否可安装以授权交付物和当前源码目录为准。

## 源码与发布包边界

| 场景 | 正确入口 | 边界 |
|------|----------|------|
| 本地开发 | `./bin/smart.php` 或 `./bin/smart.php watch` | 默认 watch，只用于源码期开发。 |
| 生产前台服务 | `./bin/smart.php start` 或发布二进制 `start` | 必须由 systemd、Supervisor 或容器编排托管。 |
| 本地建表初始化 | `composer setup` | 开发期会执行迁移、菜单同步和权限节点同步。 |
| 发布安装升级 | `xadmin:release:install` / `xadmin:release:restore --install` | 使用 release 安装包，不在正式 Phar/SFX 中执行 migrate。 |
| 插件安装更新 | 源码/CI 环境的 `xadmin:plugin:*` | 安装后重新执行前端构建；发布二进制不注册这些命令。 |
