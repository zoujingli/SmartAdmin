...
审计日志遵循租户边界：登录态、插件账号、开放应用、站点和回调能解析到可信租户时，操作日志必须记录真实 `tenant_id > 0`；解析不到可信租户时，只有 `system_logs_action` 可以记录 `tenant_id = 0` 的平台日志，并且仅 `APP_SUPER_USER` 平台超级管理员可见。`system_logs_change` 不记录平台 0 明细，避免无归属事件被误当作默认租户或普通业务变更。
