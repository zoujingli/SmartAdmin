...
`plugin/Builder` 提供 `xadmin:build:phar`，`composer build` 会委托 `.php-sfx-packer.php build` 复用并校验已有 `web/dist`，再执行菜单/节点同步、生产依赖安装、发布快照、容器缓存预编译、Phar 打包、SFX 合并、构建审计和开发依赖恢复。前端产物需先通过 `composer web:build` 或 CI 等价步骤生成，不提交到 Git。
