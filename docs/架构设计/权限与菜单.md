...
权限码不是租户切换能力。普通业务接口只在登录账号所属租户内执行，前端不展示、不提交 `tenant_id`；后端标准 CRUD 也不能把请求体 `tenant_id` 当成业务归属。`tenant_id = 0` 只表示未归属或历史待修复数据，不表示平台空间。`system.tenant.*` 和平台用户管理等管控面能力只表示平台超级管理员可以代维护目标租户资源，代码中必须显式移除租户全局范围、补已校验的 `tenant_id > 0` 条件，并通过明确权限和数据范围限制操作对象。

### SaaS 子超管

`system_user.super = 1` 是租户内子超管标识，不替代 `APP_SUPER_USER` 平台超级管理员。子超管运行时自动拥有所有非平台保留权限码，菜单和按钮也按这些权限码裁剪；平台保留权限固定为 `system.tenant.*`、`system.menu.*`、`system.setting.*`、`system.data.*`，以及跨租户用户代维护能力。`system.file.upload-config` 属于租户级上传配置能力，不归入平台保留权限，子超管可维护本租户上传通道。

子超管只能在当前 `TenantContext` 对应租户内获得全部数据范围。它不会拿到 `*`，不能看到租户管理、菜单管理、系统参数、系统数据等平台保留入口，也不能设置或取消 `system_user.super`。只有 `APP_SUPER_USER` 平台超级管理员可以在用户管理入口查看和配置 `super` 状态；非平台账号也不能通过编辑、禁用、删除、恢复、重置密码或重分配关系来操作其他子超管账号。

开放接口和外部回调不进入 System RBAC，也不接收租户切换参数。它们必须按已校验的应用、站点、账号或配置恢复租户上下文，再使用对应业务权限或 scope 判断可访问资源。
...
- 支持租户的业务页面不暴露租户选择；只有平台代维护页面可以按目标租户加载选项和提交目标租户。
- 不把平台保留权限授予租户子超管；新增平台管控功能时要确认是否需要加入子超管保留清单。
