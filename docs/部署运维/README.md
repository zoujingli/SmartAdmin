...
`部署运维` 是 SmartAdmin 部署、发布、升级和回滚的主线文档。遇到与 [运维手册](../运维手册/README.md) 重叠的内容时，以本章和当前代码实现为准；运维手册只作为缓存、日志、迁移排障等专题补充。
...
| 命令类型 | 使用环境 | 说明 |
|----------|----------|------|
| 开发启动 | 源码本地 | `./bin/smart.php` 默认进入 watch，只用于开发调试。 |
| 生产启动 | 源码部署或发布二进制 | 必须显式 `start`，并由 systemd、Supervisor 或容器编排托管。 |
| 迁移建表 | 源码开发/CI | `migrate`、fresh、rollback、seed 只作为开发期入口，正式 Phar/SFX 不注册。 |
| 插件管理 | 源码/CI | `xadmin:plugin:*` 只用于 ZIP 打包、安装、备份、移除和恢复；生产二进制不支持动态插件管理。 |
| 发布安装 | 生产/预发 | 使用 `xadmin:release:install` 或 `xadmin:release:restore --install`，先 dry-run 再正式执行。 |
| 前端构建 | 源码/CI | 插件页面变化后执行 `composer web:build`；发布包只复用已有 `web/dist`。 |
...
docs 静态站与后端服务分开部署。后端负责 API 和管理端，docs 负责全生态文档；会员授权插件仅公开说明，不随开源源码或 ZIP 分发。
