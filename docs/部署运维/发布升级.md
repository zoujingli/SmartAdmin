...
| `build:snapshot` | 在 fresh 发布临时库生成 `storage/extra/release` 安装包 |
...
`composer build` 实际委托 `.php-sfx-packer.php build`，该编排器会先复用并校验已有 `web/dist/index.html`，再在 fresh SQLite/MySQL 发布临时库完成迁移、菜单/节点同步、双方言结构捕获、双目标恢复预检和 staging 原子发布，随后安装生产依赖、生成预编译缓存、打包多架构 SFX 并执行审计。临时库通过子进程环境变量使用，原 `.env` 与原数据库不修改，并在正常和异常路径清理。`build:*` 分阶段脚本用于 CI 或人工分段调试；前端产物需要在打包前通过 `composer web:build` 或 CI 等价步骤提供。
