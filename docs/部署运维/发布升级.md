...
`build/upgrade/start.sh` 是与现有 1Panel/Docker 部署方式一致的二进制替换升级模板。脚本默认读取同目录的 `system`，上传到远端 `/tmp/system`，替换容器宿主机上的程序文件后执行：

```bash
docker restart <container>
<binary> --self xadmin:release:restore --install
<binary> --self xadmin:website:publish
docker restart <container>
```

脚本不主动执行全量备份、dry-run 或 `--force`；需要按环境调整服务器、远端路径或容器名时，优先使用脚本顶部变量或对应环境变量。真实服务器账号、密码和环境专用脚本不得提交到仓库。
