...
- **插件化分层设计** — Library/System 作为基础插件，SmartAdminBuilder 作为独立构建包，业务能力通过 `plugin/<Business>` 接入，Web 仅提供通用壳与共享运行库
