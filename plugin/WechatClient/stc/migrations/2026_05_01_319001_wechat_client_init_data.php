...
return new class extends Migration {
    ...
    private function syncPluginMenus(): void
    {
        ...
        echo 'Initialized wechat client menus: ' . count($rows) . "\n";
    }
    ...
    private function syncPluginNodes(): void
    {
        ...
        // 插件权限节点来自菜单编码与 #[Auth] 注解；初始化时整体同步，确保拆分后的控制器引用立即可用。
        $report = (new AuthRegistryService())->syncWithReport(true, true, false);
        echo sprintf(
            "Initialized wechat client auth nodes: added %d, updated %d, disabled %d\n",
            $report['added'],
            $report['updated'],
            $report['disabled']
        );
