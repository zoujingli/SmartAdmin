...
final class SystemMenuSeed
{
    ...
    public static function rows(int $userId, ?string $now = null): array
    {
        ...
        return [
            ...
            self::menu($userId, $now, 185, 200, '任务管理', 'system.scheduler.task.index', '/system/scheduler/task', '@plugin/System/views/scheduler/task/index.vue', 'lucide:calendar-clock', 22),
            self::button($userId, $now, 1851, 185, '新增任务', 'system.scheduler.task.create', 10),
            self::button($userId, $now, 1852, 185, '编辑任务', 'system.scheduler.task.update', 20),
            self::button($userId, $now, 1853, 185, '删除任务', 'system.scheduler.task.delete', 30),
            self::button($userId, $now, 1854, 185, '更新任务状态', 'system.scheduler.task.status', 40),
            self::button($userId, $now, 1855, 185, '立即执行任务', 'system.scheduler.task.run', 50),
            self::button($userId, $now, 1856, 185, '查看执行日志', 'system.scheduler.log.index', 60),
