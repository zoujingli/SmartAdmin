...
function shouldAutoRedirect(route: RouteRecordRaw) {
  if (route.redirect || !route.children || route.children.length === 0) {
    return false;
  }

  const typeCode = String(
    route.meta?.typeCode ?? (route as Record<string, any>).typeCode ?? (route as Record<string, any>).type ?? '',
  );
